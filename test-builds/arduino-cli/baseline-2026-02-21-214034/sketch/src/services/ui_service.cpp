...
void drawOperationSideFade(app::OperationMode operation) {
  ...
  for (int x = 0; x < kFadeWidth; ++x) {
    ...
    const uint16_t color = scaleColor565(accent, amount, denom);
...
void drawChip(int x, int y, int w, int h, const char* text, bool focused, bool editing = false, uint8_t font = 1) {
...
void drawVolumeHud(const app::AppState& state) {
  ...
  const int fillW = (static_cast<int>(volume) * (w - 20)) / 63;
...
void drawSettingsScreen(const app::AppState& state) {
  ...
  for (uint8_t i = 0; i < app::settings::kItemCount; ++i) {
    ...
    const bool itemEditable = app::settings::itemEditable(item);
...
void drawScreen(const app::AppState& state) {
  ...
  drawChip(fineRect.x, fineRect.y, fineRect.w, fineRect.h, fineText, focusFine, popupOpen && focusFine, 1);
  ...
  g_spr.drawString(operationName(state.ui.operation), fineRect.x + (fineRect.w / 2), fineRect.y + fineRect.h + 7);

  drawChip(modeRect.x, modeRect.y, modeRect.w, modeRect.h, modulationName(state.radio.modulation), focusMode, popupOpen && focusMode, 2);
