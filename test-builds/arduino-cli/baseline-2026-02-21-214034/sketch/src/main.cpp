...
uint16_t wrapFrequencyKhz(int32_t frequencyKhz, uint16_t minKhz, uint16_t maxKhz) {
  if (maxKhz < minKhz) {
    return minKhz;
  }

  const int32_t spanKhz = static_cast<int32_t>(maxKhz) - static_cast<int32_t>(minKhz) + 1;
  if (spanKhz <= 0) {
    return minKhz;
  }

  int32_t offsetKhz = (frequencyKhz - static_cast<int32_t>(minKhz)) % spanKhz;
  if (offsetKhz < 0) {
    offsetKhz += spanKhz;
  }

  return static_cast<uint16_t>(static_cast<int32_t>(minKhz) + offsetKhz);
}
...
void changeFrequency(int8_t direction, int8_t repeats) {
  ...
  if (app::isSsb(g_state.radio.modulation)) {
    ...
    int32_t nextBfoHz = static_cast<int32_t>(g_state.radio.bfoHz) +
                        static_cast<int32_t>(direction) * static_cast<int32_t>(repeats) * app::kBfoStepHz;
    ...
    g_state.radio.frequencyKhz = wrapFrequencyKhz(nextFrequencyKhz, bandMinKhz, bandMaxKhz);
  ...
  const int32_t deltaKhz = static_cast<int32_t>(stepKhz) * static_cast<int32_t>(direction) * static_cast<int32_t>(repeats);
  const uint16_t nextFrequencyKhz = wrapFrequencyKhz(static_cast<int32_t>(g_state.radio.frequencyKhz) + deltaKhz,
                                                     bandMinKhz,
                                                     bandMaxKhz);

  if (nextFrequencyKhz == g_state.radio.frequencyKhz) {
    return;
  }

  g_state.radio.frequencyKhz = nextFrequencyKhz;
  applyRadioState(false);
  scheduleTunePersist();
}
...
void moveQuickEditFocus(int8_t direction) {
  g_state.ui.quickEditItem = app::quickedit::moveFocus(g_state.ui.quickEditItem, direction);
...
void applyActiveSettingsValue(uint8_t valueIndex) {
  ...
  if (!app::settings::itemEditable(item)) {
    return;
  }
