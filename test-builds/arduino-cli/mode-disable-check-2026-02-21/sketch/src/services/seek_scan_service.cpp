#line 1 "/Users/beegee/Documents/ats mini/ats-mini-UNLTD/ats-mini-new/src/services/seek_scan_service.cpp"
#include <Arduino.h>
...
constexpr uint8_t kMaxRawHits = 128;
...
bool g_prevAboveThreshold = false;
...
bool isInBroadcastSwRedLine(uint16_t frequencyKhz) {
  for (size_t i = 0; i < app::kBroadcastRedLineSwCount; ++i) {
    if (app::isWithin(frequencyKhz, app::kBroadcastRedLineSw[i])) {
      return true;
    }
  }
...
const app::SubBandDef* broadcastSwSegmentFor(uint16_t frequencyKhz) {
  for (size_t i = 0; i < app::kBroadcastRedLineSwCount; ++i) {
    if (app::isWithin(frequencyKhz, app::kBroadcastRedLineSw[i])) {
      return &app::kBroadcastRedLineSw[i];
    }
  }
  return nullptr;
}

bool isAmateurBand(const app::BandId bandId) {
  switch (bandId) {
    case app::BandId::HAM160m:
    case app::BandId::HAM80m:
    case app::BandId::HAM60m:
    case app::BandId::HAM40m:
    case app::BandId::HAM30m:
    case app::BandId::HAM20m:
    case app::BandId::HAM17m:
    case app::BandId::HAM15m:
    case app::BandId::HAM12m:
    case app::BandId::HAM10m:
      return true;
    default:
      return false;
  }
...
void buildScanSegments(const app::AppState& state) {
  ...
  // For non-ALL SW bands: if current frequency is in a broadcast red-line segment,
  // and the selected band is not amateur, scan only that broadcast segment.
  if (!isAmateurBand(band.id) && isInBroadcastSwRedLine(state.radio.frequencyKhz)) {
    const app::SubBandDef* segment = broadcastSwSegmentFor(state.radio.frequencyKhz);
    if (segment != nullptr) {
      const uint16_t minKhz = segment->minKhz > bandMinKhz ? segment->minKhz : bandMinKhz;
      const uint16_t maxKhz = segment->maxKhz < bandMaxKhz ? segment->maxKhz : bandMaxKhz;
      if (minKhz <= maxKhz) {
  ...
  // Outside broadcast red-line segments, or in amateur bands: full selected band.
...
uint8_t mergeRawHits(uint16_t mergeDistanceKhz, FoundStation* merged, uint8_t maxMerged) {
  ...
  for (uint8_t i = 1; i < g_rawHitCount; ++i) {
    ...
    const uint16_t delta = hit.frequencyKhz > clusterLastFrequency ? hit.frequencyKhz - clusterLastFrequency : 0;

    if (delta <= mergeDistanceKhz) {
      clusterLastFrequency = hit.frequencyKhz;
      if (hit.rssi > clusterPeak.rssi) {
        clusterPeak = hit;
      }
      continue;
    }
...
bool tick(app::AppState& state) {
  ...
  if (aboveThreshold && !g_prevAboveThreshold && g_rawHitCount < kMaxRawHits) {
    g_rawHits[g_rawHitCount++] = {g_scanCurrentKhz, rssi, snr};
  }
  g_prevAboveThreshold = aboveThreshold;
