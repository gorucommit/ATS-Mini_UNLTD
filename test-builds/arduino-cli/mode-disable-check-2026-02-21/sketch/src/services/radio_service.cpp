...
uint16_t snapToSeekSpacing(uint16_t frequencyKhz, uint16_t minKhz, uint16_t maxKhz, uint8_t spacingKhz, int8_t direction) {
  if (spacingKhz == 0 || maxKhz < minKhz) {
    return frequencyKhz;
  }

  int32_t snapped = frequencyKhz;
  int32_t offset = (snapped - static_cast<int32_t>(minKhz)) % spacingKhz;
  if (offset < 0) {
    offset += spacingKhz;
  }
