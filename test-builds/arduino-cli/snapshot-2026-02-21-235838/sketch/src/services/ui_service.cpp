...
uint8_t signalThresholdRssi(app::Modulation modulation) {
  return modulation == app::Modulation::FM ? 5 : 10;
}

uint8_t signalThresholdSnr(app::Modulation modulation) {
  return modulation == app::Modulation::FM ? 2 : 3;
}

uint8_t signalMaxRssi(app::Modulation modulation) {
  (void)modulation;
  return 127;
}

uint8_t signalMaxSnr(app::Modulation modulation) {
  (void)modulation;
  return 127;
}

int barsForMetric(uint8_t value, uint8_t threshold, uint8_t maxValue, int slotCount) {
  if (slotCount <= 0 || maxValue <= threshold || value <= threshold) {
    return 0;
  }

  const int numerator = (static_cast<int>(value) - static_cast<int>(threshold)) * slotCount;
  const int denominator = static_cast<int>(maxValue) - static_cast<int>(threshold);
  const int bars = (numerator + denominator - 1) / denominator;
  return clampInt(bars, 0, slotCount);
}
...
void drawBottomScale(const app::AppState& state) {
  ...
  const uint8_t rssiThreshold = signalThresholdRssi(state.radio.modulation);
  const uint8_t snrThreshold = signalThresholdSnr(state.radio.modulation);
  const uint8_t rssiMax = signalMaxRssi(state.radio.modulation);
  const uint8_t snrMax = signalMaxSnr(state.radio.modulation);
  const int rssiBars = barsForMetric(g_lastRssi, rssiThreshold, rssiMax, kHalfBars);
  const int snrBars = barsForMetric(g_lastSnr, snrThreshold, snrMax, kHalfBars);
  ...
  for (int i = 0; i < kTotalBars; ++i) {
    ...
    if (i < kHalfBars) {
      ...
      if (lit) {
        barColor = i >= (kHalfBars - 3) ? kColorScaleHot : kColorRssi;
      }
    } else {
      const int fromRight = (kTotalBars - 1) - i;
      const bool lit = fromRight < snrBars;
      if (lit) {
        barColor = fromRight >= (kHalfBars - 3) ? kColorScaleHot : kColorChipFocus;
      }
    }
